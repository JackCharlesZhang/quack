...
def test_linear(in_features, out_features, input_dtype):
    ...
    m = 1920
    x = torch.randn((in_features, m), device=device, dtype=input_dtype, requires_grad=True).transpose(0,1)
    x = x[::2]  # Testing non-contiguous
    w = (
        torch.randn((in_features, out_features), device=device, dtype=input_dtype).transpose(0,1)
