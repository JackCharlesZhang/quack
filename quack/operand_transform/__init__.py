...
``transform_a=`` to :func:`quack.gemm_host.build_gemm_epi_plan` — or compose
with fn epilogues via ``EpiMod.__call__/plan/gemm(transform_a=...)`` — and it
...
host layer (it imports gemm_epilogue for the semantic keyer and class-ref
machinery, which imports the per-arch kernel classes for minting), while
transform_a.py is KERNEL-SIDE code imported BY GemmSm90. The frontend
re-exports below are lazy (PEP 562) so the kernel can import this package
without a cycle.
"""
...
def __getattr__(name):
    if name in _FRONTEND:
        from quack.operand_transform import frontend

        return getattr(frontend, name)
    if name in _HOST:
        from quack.operand_transform import host
