...
def make_smem_layout_tma_gather_a(
    ...
    *,
    ...
    is_k_major = tiled_mma.op.a_major_mode == OperandMajorMode.K
    assert is_k_major, "TMA gather A only supports K-major layout for tensor A"
    a_smem_shape = tiled_mma.partition_shape_A(
        cute.dice(mma_tiler_mnk, (1, None, 1), loc=loc, ip=ip)
    )
    a_smem_shape_mn_k = (
    ...
    a_smem_layout_atom = cute.make_composed_layout(
        a_smem_layout_atom.inner,
        0,
        cute.make_layout(
            (4, a_smem_layout_atom.outer.shape[1]),
            stride=a_smem_layout_atom.outer.stride,
            loc=loc,
            ip=ip,
        ),
        loc=loc,
